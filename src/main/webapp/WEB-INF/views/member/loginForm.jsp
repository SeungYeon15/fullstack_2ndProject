...
function find(){
  ...
  function findID(){
	  mname = document.getElementById('first').value;
	  email = document.getElementById('second').value;
	  return fetch("/member/findID/"+mname+"/"+email)
      .then(response => response.json())
      .then(data=>{
    	  try{alert(data[0].id)}
    	  catch(e){alert("일치하는 회원이 없습니다.")}});
  }
  
  function findPass(){
	  mname = document.getElementById('first').value;
	  id = document.getElementById('second').value;
	  return fetch("/member/findPass/"+mname+"/"+id)
      .then(response => response.json())     
      .then(data=>{
    	  try{alert(data[0].passwd)}
    	  catch(e){alert("일치하는 회원이 없습니다.")}});
  }
...
<body>
	<div class="container mt-5 mx-9">
		...
		<form class="form-horizontal" action="/member/login" method="post">
			...
			<input type="hidden" name="cateno" value="${param.cateno}"> <input
			...
			<button type="button" data-bs-toggle="collapse"
				data-bs-target="#demo" onclick="change(1)"
				class="btn btn-outline-dark">아이디 찾기</button>
			<button type="button" data-bs-toggle="collapse"
				data-bs-target="#demo" onclick="change(2)"
				class="btn btn-outline-dark">패스워드 찾기</button>
